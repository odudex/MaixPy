...
static bool decode_word_optimized(const char *word, size_t word_len, uint8_t *output) {
    ...
    // Convert to lowercase and get indices
    ...
    // For 4-character words, verify middle characters
    if (word_len == 4) {
...
static size_t partition_string(const char *str, size_t word_len, char ***parts, size_t max_parts) {
...
bool bytewords_encode(bytewords_style_t style, const uint8_t *data, size_t data_len, char **encoded) {
    ...
    if (style == BYTEWORDS_STYLE_MINIMAL) {
        // Minimal style: first + last character of each word
        ...
        *pos = '\0';
        ...
        // Standard or URI style: full words with separators
        char separator = (style == BYTEWORDS_STYLE_STANDARD) ? ' ' : '-';
        size_t encoded_len = total_len * 4 + (total_len - 1) + 1; // words + separators + null
...
// Raw bytewords functions (no CRC32) for internal UR use
bool bytewords_decode_raw(bytewords_style_t style, const char *encoded, uint8_t **decoded, size_t *decoded_len) {
    ...
    // Decode words to bytes (no CRC validation)
    uint8_t *buf = safe_malloc(num_words);
    if (!buf) {
        free_string_array(words, num_words);
        free(words);
...
bool bytewords_encode_raw(bytewords_style_t style, const uint8_t *data, size_t data_len, char **encoded) {
    ...
    if (style == BYTEWORDS_STYLE_MINIMAL) {
        ...
        *pos = '\0';
        ...
        // Standard or URI style: full words with separators
        char separator = (style == BYTEWORDS_STYLE_STANDARD) ? ' ' : '-';
        size_t encoded_len = data_len * 4 + (data_len - 1) + 1; // words + separators + null
        *encoded = safe_malloc(encoded_len);
        if (!*encoded) return false;
