...
static uint8_t sd_get_csdregister(SD_CSD *SD_csd)
{
    ...
    {
        printk("[MaixPy] SD_START_DATA_SINGLE_BLOCK_READ %s | resp = %x \r\n", __func__, resp);
        sd_end_cmd();
        return 0xFF;
    }
...
static uint8_t sd_get_cidregister(SD_CID *SD_cid)
{
    ...
    {
        printk("fail sd_get_cidregister != 0x00");
        sd_end_cmd();
        return 0xFF;
    }
    if (sd_get_response() != SD_START_DATA_SINGLE_BLOCK_READ)
    {
        printk("fail sd_get_cidregister != SD_START_DATA_SINGLE_BLOCK_READ");
        sd_end_cmd();
        return 0xFF;
    }
...
static uint8_t sd_get_cardinfo(SD_CardInfo *cardinfo)
{
    ...
    {
        printk("[MaixPy] %s | sd_get_csdregister failed SD_csd\r\n",__func__);
        return 0xFF;
    }
    if (sd_get_cidregister(&(cardinfo->SD_cid)))
    {
        printk("[MaixPy] %s | sd_get_cidregister failed SD_cid\r\n",__func__);
        return 0xFF;
    }
    if (2 == sd_version)
    {
...
uint8_t sd_init(void) {
  ...
  /*!< SD initialized and set to SPI mode properly */
    ...
    if (1 == sd_version)
    {
        ...
        if (sd_get_response() != 0x00)
        {
            printk("sd_send_cmd(SD_CMD16, 512); if (sd_get_response() != 0x00)\r\n");
            sd_end_cmd();
            return 0xFF;
        }
    }
